...
if __name__ == '__main__':
    try:
        k = int(input('please, enter the size of the group: '))
        if k <= 1:
            print('wrong input')
            exit(code=-1)
        else:
            rooms = [int(i) for i in input('please, enter the elements of the room number list: ').split()]
            captain = set()
            families = set()
            for r in rooms:
                if r in captain:
                    ...
                    captain.add(r)
            for c in captain.difference(families):
                print(c)
    except ValueError:
